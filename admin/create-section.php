...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $create_section_query = "INSERT INTO sections (name, course_id, staff_id) VALUES ('$name', '$course_id', '$staff_id')";
    $create_section_result = mysqli_query($conn, $create_section_query);
    if ($create_section_result) {
        $message = 'Section has been successfully created!';
    } else {
        $error = 'Unable to create the section!';
    }
}
...
<head>
    ...
    <title>Create Section &mdash; EduRV</title>
                                    ...
                                    <div class="card-body">
                                        <form action="" method="POST" class="needs-validation" onsubmit="create()">
                                            ...
                                            <div class="form-group row mb-4">
                                                ...
                                                <div class="col-sm-12 col-md-7">
                                                    <select id="staff-id" class="form-control" name="staff_id" required autofocus>
                                                        <option selected disabled>Choose Staff</option>
                                                        <?php
                                                        foreach ($staffs as $staff) {
                                                            echo '<option value="' . $staff['id'] . '"' . ' data-course="' . $staff['course_id'] . '">' . $staff['name'] . '</option>';
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
