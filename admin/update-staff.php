...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_staff_query = "UPDATE staffs SET name = '$name', email = '$email', mobile_number = '$mobile_number', program_id = '$program_id', course_id = '$course_id' WHERE id = '$staff_id'";
    ...
    if ($update_staff_result) {
        ...
        $error = 'Unable to update the staff!';
    }
}

$staff;
$staff_query = "SELECT * FROM staffs WHERE id = '$staff_id'";
$staff_result = mysqli_query($conn, $staff_query);

if ($staff_result && mysqli_num_rows($staff_result) == 1) {
...
<head>
    ...
    <title>Update Staff &mdash; EduRV</title>
                                        ...
                                        <form action="" method="POST" class="needs-validation" onsubmit="update()">
                                            <div class="form-group row mb-4">
                                                ...
                                                <div class="col-sm-12 col-md-7">
                                                    <input class="form-control" name="name" value="<?php echo $staff['name'] ?>" required autofocus>
                                                </div>
                                            </div>
                                            <div class="form-group row mb-4">
                                                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Staff Email</label>
                                                <div class="col-sm-12 col-md-7">
                                                    <input class="form-control" name="email" type="email" value="<?php echo $staff['email'] ?>" required autofocus>
                                                </div>
                                            </div>
                                            <div class="form-group row mb-4">
                                                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Staff Mobile Number</label>
                                                <div class="col-sm-12 col-md-7">
                                                    <input class="form-control" name="mobile_number" type="tel" minlength="10" maxlength="10" value="<?php echo $staff['mobile_number'] ?>" required autofocus>
                                                </div>
                                            </div>
                                            <div class="form-group row mb-4">
                                                ...
                                                <div class="col-sm-12 col-md-7">
                                                    <select class="form-control" name="program_id" required autofocus>
                                                        ...
                                                        foreach ($programs as $program) {
                                                            echo '<option value="' . $program['id'] . '" ' . ($staff['program_id'] == $program['id'] ? "selected" : "") . '>' . $program['name'] . '</option>';
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
                                            ...
                                            <div class="form-group row mb-4">
                                                ...
                                                <div class="col-sm-12 col-md-7">
                                                    <select id="course-id" class="form-control" name="course_id" value="<?php echo $staff['course_id'] ?>" required autofocus>
                                                        <option disabled>Choose Course</option>
                                                        <?php
                                                        foreach ($courses as $course) {
                                                            echo '<option value="' . $course['id'] . '" data-semester="' . $course['semester'] . '" ' . ($staff['course_id'] == $course['id'] ? "selected" : "") . '>' . $course['code'] . ' - ' . $course['name'] . '</option>';
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
